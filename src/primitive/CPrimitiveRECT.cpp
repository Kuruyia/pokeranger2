...
void CPrimitiveRECT::SetRectangle(fx32 x, fx32 y, fx32 w, fx32 h, fx32 z)
{
    m_rectangle.pos.x = x;
    m_rectangle.pos.y = y;
    m_rectangle.size.x = w;
    m_rectangle.size.y = h;
    m_z = z;
}

void CPrimitiveRECT::sub_02003C00(Point2D *arg0, CPrimitiveRECT *arg1)
{
    Point2D v0(arg1->m_rectangle.pos);
    *arg0 = v0;
}
...
u32 CPrimitiveRECT::RenderWithTexture(CPrimitive_Render_arg *arg0)
{
    ...
    fx32 halfW = m_rectangle.size.x >> 1;
    fx32 halfH = m_rectangle.size.y >> 1;
    ...
    G3_Vtx(FX_Whole(m_rectangle.pos.x - halfW) << v2, FX_Whole(m_rectangle.pos.y - halfH) << v2, m_z);

    G3_TexCoord(m_texCoords.x1, m_texCoords.y2);
    G3_VtxXY(FX_Whole(m_rectangle.pos.x - halfW) << v2, FX_Whole(m_rectangle.pos.y + halfH) << v2);
...
u32 CPrimitiveRECT::RenderWithoutTexture(CPrimitive_Render_arg *arg0)
{
    ...
    fx32 halfW = m_rectangle.size.x / 2;
    fx32 halfH = m_rectangle.size.y / 2;
    ...
    G3_Vtx(FX_Whole(m_rectangle.pos.x - halfW) << v0, FX_Whole(m_rectangle.pos.y - halfH) << v0, m_z);
    G3_VtxXY(FX_Whole(m_rectangle.pos.x - halfW) << v0, FX_Whole(m_rectangle.pos.y + halfH) << v0);
    G3_VtxXY(FX_Whole(m_rectangle.pos.x + halfW) << v0, FX_Whole(m_rectangle.pos.y + halfH) << v0);
    G3_VtxXY(FX_Whole(m_rectangle.pos.x + halfW) << v0, FX_Whole(m_rectangle.pos.y - halfH) << v0);
